...
contract TokenLock is Ownable {
    ...
    /// @notice Allows the releaseaddress to claim the contract's vested tokens. Errors if no tokens have vested
    ...
    /// @notice Terminate token Timelock transferring all vested tokens to the `_recipient`
    /// and returning all non-vested tokens to the contract owner
    /// Secured to the contract owner only
    /// @param _recipient address of the tokenRelease recipient
    function revokeTokenRelease(address _recipient) 
        external 
        onlyOwner
    {
    ...
    /// @notice Calculate the vested and unclaimed months and tokens available for `_releaseaddress` to claim
    /// Due to rounding errors once gate duration is reached, returns the entire left gate amount
    ...
    function calculateTokenClaim(address _recipient) private view returns (uint16, uint256) {
        ...
        // For gatess created with a future start date, that hasn't been reached, return 0, 0
